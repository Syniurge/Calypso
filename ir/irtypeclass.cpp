...
void IrTypeClass::addBaseClassData(AggrTypeBuilder &builder,
                                   AggregateDeclaration *baseAgg) {
  // CALYPSO
  if (auto lp = baseAgg->langPlugin())
  {
    lp->codegen()->addBaseClassData(builder, baseAgg);
    return;
  }

  assert(baseAgg->isClassDeclaration());
  ClassDeclaration *base = static_cast<ClassDeclaration*>(baseAgg);
...
void IrTypeClass::addInterfaceToMap(ClassDeclaration *inter, size_t index) {
  ...
  if (inter->interfaces_dim > 0) {
    ...
    addInterfaceToMap(static_cast<ClassDeclaration *>(b->base), index);
  }
}
