...
else
{
     ...
     */
    final void removeInstance(TemplateInstance ti)
    {
 ...
 */
void functionResolve(Match* m, Dsymbol dstart, Loc loc, Scope* sc, Objects* tiargs, Type tthis, Expressions* fargs, int flags = 0) // CALYPSO
{
    version (none)
    {
    ...
    final Identifier genIdent(Objects* args)
    {
        ...
        for (size_t i = 0; i < args.dim; i++)
        {
            ...
            if (!insideTuple)
            {
                ...
                // DMD BUG: the original code is wrong when i and args get switched by "else if (va)" below
