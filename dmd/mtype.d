...
version (IN_LLVM)
{
    ...
    ushort sizeType() const nothrow // CALYPSO
    {
        return sizeTy[ty];
    }
...
//         t.equivDeco = null; // CALYPSO
    ...
    final void check()
    {
        ...
        if (tn && ty != Tfunction && tn.ty != Tfunction && ty != Tenum)
        {
            ...
            case MODFlags.shared_ | MODFlags.wildconst:
            case MODFlags.immutable_:
                assert(!isTransitive() || tn.mod == MODFlags.immutable_ || (tn.mod & mod) == mod); // CALYPSO
                break;
...
extern (C++) class TypeBasic : Type // CALYPSO (final removed)
{
    ...
    override d_uns64 size(const ref Loc loc) // CALYPSO (const remove
    {
        uint size;
    ...
    override bool isfloating() // CALYPSO (const removed from isxxx functions)
    {
    ...
    override bool isZeroInit(const ref Loc loc) // CALYPSO (const removed)
    {
        switch (ty)
        {
        case Tchar:
...
extern (C++) class TypePointer : TypeNext // CALYPSO (removed final)
{
    ...
    override d_uns64 size(const ref Loc loc) // CALYPSO (const removed)
    {
        return target.ptrsize;
    }
...
extern (C++) final class TypeClass : Type
{
    ...
    extern (D) MATCH implicitConvToWithoutAliasThis(Type to)
    {
        ...
        if (adto)
        {
            ...
            if (cdto && cdto.semanticRun < PASS.semanticdone && !cdto.isBaseInfoComplete())
    ...
    override MATCH constConv(Type to)
    {
        if (equals(to))
            return MATCH.exact;
        if (ty == to.ty && sym == (cast(TypeClass)to).sym && (MODimplicitConv(mod, to.mod) || !byRef())) // CALYPSO HACK:
                     // TypeStruct returns MATCH.const for mutable -> immutable conv,
    ...
    override Expression defaultInitLiteral(const ref Loc loc) // CALYPSO
    {
        if (!byRef()) // hmm?
        {
            auto e = sym.defaultInitLiteral(loc); // CALYPSO
