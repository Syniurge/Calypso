...
//            tf.isscope = true;                                  // return by itself means 'return scope'
        ...
        if (tf.next)
        {
            ...
            {
                auto tref = cast(TypeReference)tf.next;
        ...
        if (tf.parameterList.parameters)
        {
            ...
            for (size_t i = 0; i < dim; i++)
            {
                ...
                {
                    fparam.storageClass |= STC.nodtor; // CALYPSO see D20180120T151603
                }
...
Type merge(Type type)
{
    switch (type.ty)
    {
        ...
        default:
            ...
            {
                (cast(TypeNext)type).transitive(); // CALYPSO
                if (!type.nextOf().deco)
                    return type;
            }
            break;
    ...
    Expression visitStruct(TypeStruct mt)
    {
        ...
        if (e.type)
            e.type = mt; // restore the type qualifiers if any
                // NOTE: the cpp:: version of TypeStruct::defaultInit creates a CallExp(TypeExp) that needs its .type set to null (LDC 1.17 TODO: shouldn't it return a init literal if there's no default ctor?)
        return e;
    }

    Expression visitClass(TypeClass mt) // CALYPSO
    {
        static if (LOGDEFAULTINIT)
        {
