 ...
 */
Expression callCpCtor(Scope* sc, Expression e, Type destinationType) // CALYPSO (made public)
{
    Type tv = e.type.baseElemOf();
    if (tv.isAggregateValue()) // CALYPSO
    {
        if (auto lp = tv.getAggregateSym().langPlugin())
            if (auto e2 = lp.callCpCtor(sc, e)) // FIXME use destinationType
                return e2;
...
version (IN_LLVM)
{
    ...
    extern (D) this(const ref Loc loc, AggregateDeclaration sd, Expressions* elements, Type stype = null) // CALYPSO
    {
