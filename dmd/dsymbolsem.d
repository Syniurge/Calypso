...
extern(C++) final class DsymbolSemanticVisitor : Visitor // CALYPSO (made public)
{
    ...
    override void visit(TemplateDeclaration tempdecl)
    {
        ...
        for (size_t i = 0; i < tempdecl.parameters.dim; i++)
        {
            ...
            if (i + 1 != tempdecl.parameters.dim && tp.isTemplateTupleParameter() && !tempdecl.allowTupleParameterAnywhere()) // CALYPSO
            {
    ...
    void funcDeclarationSemantic(FuncDeclaration funcdecl)
    {
        ...
        if (ClassDeclaration cd = parent.isClassDeclaration())
        {
            ...
            int vi = cb ? funcdecl.findVtblIndex(&cb.vtbl, cast(int)cb.vtbl.dim) : -1;
            ...
            default:
                {
                    ...
                    if (!funcdecl.isOverride())
                    {
                        ...
                        {
                            int vi2 = funcdecl.findVtblIndex(&cb.vtbl, cast(int)cb.vtbl.dim, false); // CALYPSO
                            if (vi2 < 0)
        ...
        Linterfaces:
            foreach (b; cd.interfaces)
            {
                ...
                if (!icd)
                    continue;
                vi = funcdecl.findVtblIndex(&icd.vtbl, cast(int)icd.vtbl.dim);
                switch (vi)
                {
                case -1:
                    break;
                ...
                default:
                    {
                        auto fdv = cast(FuncDeclaration)icd.vtbl[vi]; // CALYPSO
                        Type ti = null;
            ...
            if (!doesoverride && funcdecl.isOverride() && (funcdecl.type.nextOf() || !may_override))
            {
                ...
                if (s)
                    funcdecl.error("does not override any function, did you mean to override '%s%s'?",
                        (bc.sym.isClassDeclaration() && (cast(ClassDeclaration)bc.sym).isCPPclass()) ? "extern (C++) ".ptr : "".ptr, s.toPrettyChars()); // CALYPSO
                else
    ...
    override void visit(ClassDeclaration cldec)
    {
        ...
        if (cldec.semanticRun == PASSinit)
        {
            ...
            if (sc.linkage == LINKcpp && !cldec.langPlugin()) // CALYPSO NOTE: differentiation between DMD's extern(C++) classes and Calypso mapped C++ classes may be temporary
        ...
        if (cldec.baseok < BASEOKdone)
        {
            ...
            if (cldec.baseclasses.dim)
            {
                ...
                if (!sym || (sym.isStructDeclaration() && !cldec.allowInheritFromStruct()))
                {
                ...
                if (sym.isDeprecated())
                {
                    if (!cldec.isDeprecated())
                    {
                        ...
                        cldec.isdeprecated = true;
                        tb.checkDeprecated(cldec.loc, sc);
                    }
                }
                if (sym.isInterfaceDeclaration())
                    goto L7;

                for (AggregateDeclaration cdb = sym; cdb; cdb = toAggregateBase(cdb)) // CALYPSO
                {
                    if (cdb == cldec)
                    {
                ...
                if (!bcd || bcd.baseok < BASEOKdone) // CALYPSO NOTE: is it necessary to do it for a Struct base? this is how it was done in pre-DDMD Calypso, so for now
            ...
            for (size_t i = (cldec.baseClass ? 1 : 0); i < cldec.baseclasses.dim;)
            {
                ...
                Type tb = b.type.toBasetype();
                auto sym = getAggregateSym(tb); // CALYPSO
                if (tb.ty == Tstruct && !cldec.allowInheritFromStruct())
                    sym = null;
            ...
            if (!cldec.baseClass && cldec.ident != Id.Object && !cldec.classKind == ClassKind.cpp && !cldec.langPlugin()) // CALYPSO
            {
                void badObjectDotD()
                {
            ...
            if (cldec.baseClass)
            {
                ...
                if (auto bcd = isClassDeclarationOrNull(cldec.baseClass)) // CALYPSO
                {
            ...
            foreach (b; cldec.interfaces)
            {
                ...
                if (bcd.isCOMinterface())
                    cldec.com = true;
                if (cldec.classKind == ClassKind.cpp && !bcd.isCPPinterface())
                {
    ...
    Lancestorsdone:
        ...
        //    this() { }
        if (!cldec.ctor && cldec.baseClass && cldec.baseClass.ctor && !cldec.langPlugin()) // CALYPSO HACK do not generate a D ctor for extern(C++) classes mapped by Calypso
        {
        ...
        {
            if (cldec.byRef() && !(f.storage_class & STCdisable)) // CALYPSO
        ...
        if (!cldec.langPlugin())
            markAggregateReferenced(cldec); // CALYPSO
    ...
    override void visit(InterfaceDeclaration idec)
    {
        ...
        if (idec.baseok < BASEOKdone)
        {
            ...
            foreach (b; idec.interfaces)
            {
                // If this is an interface, and it derives from a COM interface,
                // then this is a COM interface too.
                assert(b.sym.isInterfaceDeclaration()); // CALYPSO
                auto ib = cast(InterfaceDeclaration)b.sym;
                ...
                if (ib.vtblOffset())
                {
                    ...
                    if (d > 1)
                    {
                        ...
                        for (size_t j = 1; j < d; j++)
                            idec.vtbl.push(ib.vtbl[j]);
                    }
                }
                else
                {
                    idec.vtbl.append(&ib.vtbl);
                }
...
void templateInstanceSemantic(TemplateInstance tempinst, Scope* sc, Expressions* fargs)
{
    ...
    // Copy the syntax trees from the TemplateDeclaration
    tempinst.members = tempdecl.copySyntaxTree(tempinst); // CALYPSO
    ...
    if ((sc.func || (sc.flags & SCOPEfullinst) || tempinst.langPlugin()) && !tempinst.tinst) // CALYPSO 0.17 HACK FIXME: sc->func gets reset if too deep during DeclReferencer and some times that resulted in functions not getting semantic3'd.. is this the correct fix?
    {
