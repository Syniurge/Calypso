...
extern(C++) final class DsymbolSemanticVisitor : Visitor // CALYPSO (made public)
{
    ...
    override void visit(ClassDeclaration cldec)
    {
        ...
        if (cldec.semanticRun == PASS.init)
        {
            ...
            if (sc.linkage == LINK.cpp && !cldec.langPlugin()) // CALYPSO NOTE: differentiation between DMD's extern(C++) classes and Calypso mapped C++ classes may be temporary
    ...
    Lancestorsdone:
        ...
        {
            BaseClass* b = (*cldec.baseclasses)[i];
            if (b.sym.semanticRun < PASS.semanticdone) // CALYPSO
            {
                // Forward referencee of one or more bases, try again later
                cldec._scope = scx ? scx : sc.copy();
                cldec._scope.setNoFree();
        ...
        {
            if (cldec.byRef() && !(f.storage_class & STC.disable)) // CALYPSO
...
void templateInstanceSemantic(TemplateInstance tempinst, Scope* sc, Expressions* fargs)
{
    ...
    if ((sc.func || (sc.flags & SCOPE.fullinst) || tempinst.langPlugin()) && !tempinst.tinst) // CALYPSO 0.17 HACK FIXME: sc->func gets reset if too deep during DeclReferencer and some times that resulted in functions not getting semantic3'd.. is this the correct fix?
    {
