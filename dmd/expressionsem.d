    ...
    *peprefix = null;
    ...
    for (size_t i = 0; i < nargs; i++)
    {
        ...
        else
        {
            ...
            // IN_LLVM replaced: if (tf.linkage != LINK.d)
            if (tf.linkage != LINK.d && (!fd || !DtoIsIntrinsic(fd)))
            {
    ...
    if (!IN_LLVM && tf.linkage == LINK.d && tf.varargs == 1)
    {
...
private extern (C++) final class ExpressionSemanticVisitor : Visitor
{
            ...
            if (exp.op == TOK.construct)
            {
                ...
                if (sd == getAggregateSym(t2)) // CALYPSO
                {
                        ...
                        e2y.type.implicitConvTo(t1))
                    {
                        ...
                        Expression e = ae ? Expression.combine(ae, cx) : cx; // CALYPSO
                            ...
                            || sd.hasCopyCtor(sc)) // CALYPSO (not pretty)
                    {
                        ...
                        if (e2x.isLvalue())
                        {
                            ...
                            e = new DotVarExp(exp.loc, e, (cast(StructDeclaration)sd).postblit, false); // CALYPSO
    ...
    override void visit(CmpExp exp)
    {
        ...
        case TOK.uge:
            altop = TOK.greaterOrEqual;
            break;

        case TOK.ug:
            altop = TOK.greaterThan;
            break;
        ...
        if (altop != TOK.reserved)
        {
            if (!t1.isfloating())
            {
                if (altop == TOK.error)
                {
                    const(char)* s = exp.op == TOK.unord ? "false" : "true";
