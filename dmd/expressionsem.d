    ...
    Type* prettype, Expression* peprefix)
{
    ...
    *peprefix = null;
    ...
    if (!IN_LLVM && tf.linkage == LINK.d && tf.parameterList.varargs == VarArg.variadic)
    {
    ...
    override void visit(NewExp exp)
    {
        ...
        if (tb.ty == Tclass)
        {
            ...
            else
            {
                ...
                for (AggregateDeclaration c = cd; c; c = toAggregateBase(c))
                {
                    foreach (v; c.fields)
                    {
        ...
        if (sc.isD() && ad)
            if (auto lp = ad.langPlugin())
            {
                ...
                if (exp.member)
                    lp.markSymbolReferenced(exp.member);
            }
        ...
        if (t1)
        {
            ...
            if (sd && !sd.byRef()) // CALYPSO
            {
                ...
                */
                ...
                {
                    if (!sd.noDefaultCtor && !(exp.arguments && exp.arguments.dim) && !sd.langPlugin()) // CALYPSO HACK
                        goto Lx;
        ...
        else if (e.tspec)
        {
            ...
            else
            {
                ...
                for (size_t i = 1; i < e.parameters.dim; i++)
                {
                    ...
                    m = tp.matchArg(e.loc, sc, &tiargs, i, &argi, e.parameters, &dedtypes, &s);
                    if (m <= MATCH.nomatch)
                        goto Lno;
            ...
            if (exp.op == TOK.construct)
            {
                ...
                if (sd == getAggregateSym(t2)) // CALYPSO
                {
                        ...
                        e2y.type.implicitConvTo(t1))
                    {
                         ...
                         * initializer
                         */
                        Expression einit = getInitExp(cast(StructDeclaration)sd, exp.loc, sc, t1);
                        ...
                        ae = new BlitExp(exp.loc, exp.e1, einit);
                        ae.type = e1x.type;
                        ...
                        auto e = ae ? Expression.combine(ae, cx) : cx; // CALYPSO
                            ...
                            || sd.hasCopyCtor(sc)) // CALYPSO (not pretty)
                    {
                        ...
                        if (e2x.isLvalue())
                        {
                            ...
                            if ((cast(StructDeclaration)sd).hasCopyCtor) // CALYPSO FIXME
                            {
                                 ...
                                 */
                                Expression einit = new BlitExp(exp.loc, exp.e1, getInitExp(cast(StructDeclaration)sd, exp.loc, sc, t1)); // CALYPSO
                                einit.type = e1x.type;
                            ...
                            else
                            {
                                ...
                                e = new DotVarExp(exp.loc, e, (cast(StructDeclaration)sd).postblit, false);
 ...
 */
extern (C++) Expression unaSemantic(UnaExp e, Scope* sc) // CALYPSO
{
    static if (LOGSEMANTIC)
    {
