...
else
{
     ...
     */
    final FileName setOutfilename(const(char)* name, const(char)* dir, const(char)* arg, const(char)* ext)
    {
        return setOutfilename(name.toDString(), dir.toDString(), arg.toDString(), ext.toDString());
    }

    /// Ditto
    extern(D) final FileName setOutfilename(const(char)[] name, const(char)[] dir, const(char)[] arg, const(char)[] ext)
    {
     ...
     */
    final bool loadSourceBuffer(const ref Loc loc, ref File.ReadResult readResult)
    {
...
else
{
     ...
     */
    final bool read(const ref Loc loc)
    {
        if (srcBuffer)
...
version (IN_LLVM)
{
    ...
    final void checkAndAddOutputFile(const ref FileName file);
     ...
     */
    final void fullyQualifiedName(ref OutBuffer buf) // CALYPSO
    {
