    ...
    if (!t.vtinfo)
    {
        ...
        if (!builtinTypeInfo(t))
        {
            ...
            if (sc) // if in semantic() pass
            {
                ...
                Module m = sc._module.isCodegen() ? sc._module : sc._module.importedFrom; // CALYPSO semi-HACK (and not strictly necessary but makes D modules importing C++ modules smaller)
...
extern(C++) bool builtinTypeInfo(Type t)
{
    ...
    if ((t.isTypeBasic() && !t.langPlugin()) || (!IN_LLVM && t.ty == Tclass) || t.ty == Tnull) // CALYPSO
        return !t.mod;
    if (t.ty == Tarray)
    {
        Type next = t.nextOf();
