...
else
{
     ...
     */
    final const(char)* toCharsNoConstraints()
    {
        if (literal)
            return Dsymbol.toChars();
     ...
     */
    bool evaluateConstraint(TemplateInstance ti, Scope* sc, Scope* paramscope, Objects* dedargs, FuncDeclaration fd) // CALYPSO
    {
     ...
     */
    final const(char)* getConstraintEvalError(ref const(char)* tip)
    {
        import dmd.staticcond;
    ...
    private final void formatParamsWithTiargs(ref Objects tiargs, ref OutBuffer buf)
    {
     ...
     */
    final Scope* scopeForTemplateParameters(TemplateInstance ti, Scope* sc)
    {
     ...
     * Return match level.
     */
    MATCH matchWithInstance(Scope* sc, TemplateInstance ti, Objects* dedtypes, Expressions* fargs, int flag) // CALYPSO
    {
        enum LOGM = 0;
        static if (LOGM)
        {
     ...
     */
    MATCH deduceFunctionTemplateMatch(TemplateInstance ti, Scope* sc, ref FuncDeclaration fd, Type tthis, Expressions* fargs) // CALYPSO
    {
        size_t nfparams;
        size_t nfargs;
     ...
     */
    final FuncDeclaration doHeaderInstantiation(TemplateInstance ti, Scope* sc2, FuncDeclaration fd, Type tthis, Expressions* fargs) // CALYPSO
    {
        assert(fd);
        version (none)
        {
    ...
    Type tthis, Expressions* fargs, const(char)** pMessage = null, int flags = 0) // CALYPSO
{
    ...
    int applyFunction(FuncDeclaration fd)
    {
        ...
        */
        ...
        MATCH mfa = tf.callMatch(tthis_fd, fargs_, callMatchFlags, pMessage, sc); // CALYPSO
        //printf("test1: mfa = %d\n", mfa);
    ...
    int applyTemplate(TemplateDeclaration td)
    {
        ...
        if (!f)
        {
            ...
            auto tf = cast(TypeFunction)fd.type;
            MATCH mfa = tf.callMatch(tthis_fd, fargs_, callMatchFlags, null, sc); // CALYPSO
            if (mfa < m.last)
                return 0;
    ...
    if (td_best && ti_best && m.count == 1)
    {
        ...
        if (tf.ty == Terror)
            goto Lerror;
        assert(tf.ty == Tfunction);
        if (!tf.callMatch(tthis_best, fargs_, callMatchFlags, null, sc)) // CALYPSO
            goto Lnomatch;
...
private bool reliesOnTemplateParameters(Type t, TemplateParameter[] tparams)
{
    ...
    bool visitIdent(Identifier id) // CALYPSO
    {
        foreach (tp; tparams)
        {
            if (tp.ident.equals(id))
    ...
    bool visitTempArgs(Objects* tiargs) // CALYPSO
    {
        foreach (oa; *tiargs)
        {
            if (auto ta = isType(oa))
            {
                if (ta.reliesOnTemplateParameters(tparams))
                    return true;
            }
            else if (auto ea = isExpression(oa))
            {
                if (ea.reliesOnTemplateParameters(tparams))
                    return true;
            }
        }
        return false;
    ...
    bool visitTemplateInstance(TemplateInstance ti) // CALYPSO
    {
        if (visitIdent(ti.name))
            return true;
        if (ti.tiargs && visitTempArgs(ti.tiargs))
            return true;
        return false;
    }

    bool visitIdents(TypeQualified t)
    {
        foreach (o; t.idents)
        {
            if (o.dyncast() == DYNCAST.identifier &&
                            visitIdent(cast(Identifier)o))
                return true;
            else if (o.dyncast() == DYNCAST.dsymbol &&
                            visitTemplateInstance(cast(TemplateInstance)o))
                return true;
        }
        return false;
    }
    ...
    {
        if (visitIdent(t.ident)) // CALYPSO
            return true;
        return visitIdents(t);
    }

    bool visitInstance(TypeInstance t)
    {
        if (visitTemplateInstance(t.tempinst)) // CALYPSO
            return true;
        return visitIdents(t);
    }
...
extern (C++) class TemplateInstance : ScopeDsymbol
{
    ...
    final bool needsCodegen()
    {
        ...
        if (global.params.allInst)
        {
            ...
            if (enclosing)
            {
                 ...
                 */
                return !enclosing.inNonCodegen(); // CALYPSO
            }
            return true;
        }
         ...
         */
        if (enclosing && enclosing.inNonCodegen()) // CALYPSO
        {
            if (tinst)
            {
     ...
     */
    static bool semanticTiargs(const ref Loc loc, Scope* sc, Objects* tiargs, int flags) // CALYPSO
    {
    ...
    extern (D) final bool findBestMatch(Scope* sc, Expressions* fargs)
    {
        ...
        if (havetempdecl)
        {
            ...
            // TODO: Normalizing tiargs for https://issues.dlang.org/show_bug.cgi?id=7469 is necessary? // CALYPSO FIXME yeah probably
            return true;
        }
 ...
 *      i               i'th argument
 *      argi            (CALYPSO) i'th argument (not always <= i since tuple parameters may be anywhere in C++ templates)
 *      parameters[]    template parameters
 *      dedtypes[]      deduced arguments to template instance
 *      *psparam        set to symbol declared and initialized to dedtypes[i]
 */
extern (C++) MATCH matchArg(TemplateParameter tp, Loc instLoc, Scope* sc, Objects* tiargs, size_t i, size_t* argi /*CALYPSO*/, TemplateParameters* parameters, Objects* dedtypes, Declaration* psparam)
{
