...
extern (C++) final class Mangler : Visitor // CALYPSO
{
    ...
    */
    ...
    void mangleTemplateInstance(TemplateInstance ti)
    {
        ...
        for (size_t i = 0; i < args.dim; i++)
        {
            ...
            if (!insideTuple)
            {
                auto tp = ti.correspondingParam(i); // CALYPSO
            ...
            {
                assert(i + 1 == args.dim || tempdecl.allowTupleParameterAnywhere()); // must be last one // CALYPSO
                args = &va.objects;
                i = -cast(size_t)1;
                processArgs(&va.objects, true); // CALYPSO not pretty, but why and how does writeln get a Tuple in its tiargs? this seems inconsistent with the flattening in findBestMatch..
            }
            else
                assert(0);
        }
