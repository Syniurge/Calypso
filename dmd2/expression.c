...
Expression *callCpCtor(Scope *sc, Expression *e)
{
    ...
    if (tv->ty == Tstruct/* || isClassValue(tv)*/) // CALYPSO FIXME TODO
    {
...
{
    Expression::semantic(sc); // CALYPSO DMD BUG fix?
    assert(type);
    if (type->ty == Terror)
        return new ErrorExp();
    assert(type->deco);
...
//                 if (auto cpctor = static_cast<FuncDeclaration*>(ad->searchCpCtor())) CALYPSO FIXME TODO damn, shoudl've waited
                if (sd && sd->postblit)
                {
