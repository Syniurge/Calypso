...
void Module::buildTargetFiles(bool singleObj, bool library) {
  ...
  if (!objfile) {
    ...
    if (global.params.output_o) {
      ...
      objfile =
          buildFilePath(objname, global.params.objdir, global.bc_ext);
    } else if (global.params.output_ll) {
      objfile =
          buildFilePath(objname, global.params.objdir, global.ll_ext);
    } else if (global.params.output_s) {
      objfile =
          buildFilePath(objname, global.params.objdir, global.s_ext);
    }
  }
  if (doDocComment && !docfile) {
    docfile = buildFilePath(global.params.docname, global.params.docdir,
                                    global.doc_ext);
  }
  if (doHdrGen && !hdrfile) {
    hdrfile = buildFilePath(global.params.hdrname, global.params.hdrdir,
                                    global.hdr_ext);
  }
